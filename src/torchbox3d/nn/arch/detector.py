...
class Detector(MetaDetector):
    ...
    def validation_step(
        ...
        if self.debug or batch_idx % self.val_log_freq == 0 and self.local_rank == 0:
            to_logger(
                dts,
                uuids,
                data,
        ...
        for (log_id, timestamp_ns), group in dts.groupby(["log_id", "timestamp_ns"]):
            dst = (
                Path(self.dst_dir)
                / "predictions"
                / self.uuid
        ...
        losses = {f"val/{k}": v for k, v in outputs.items() if isinstance(v, Tensor)}
        self.log_dict(
            losses,
            batch_size=self.batch_size,
            sync_dist=True,
