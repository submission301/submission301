...
class DataLoader:
    ...
    def _build_annotations_metadata(self) -> None:
        ...
        if self.split_name in ("train",):
            ...
            if self.use_repeat_factor_sampling:
                ...
                #     r_i.shape,
                #     index.shape,
                #     torch.as_tensor(r_i["rep_factor"]).long().shape,
                #     indices.shape,
                #     thresholds,
                # )
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        # if self.dataset_name == "av2":
        #     metadata_dir = Path(torchbox3d.__file__).parent.parent.parent / "metadata"
        #     means = pl.read_ipc(metadata_dir / "av2_train_means.feather").select(
        #         features_column_names
        #     )
        #     stds = pl.read_ipc(metadata_dir / "av2_train_stds.feather").select(
        #         features_column_names
        #     )

        #     expr = {k: (pl.col(k) - means[k]) / stds[k] for k in features_column_names}
        #     features_pl = features_pl.with_columns(**expr)
...
def _normalize_waymo(features: np.ndarray) -> np.ndarray:
    # features[:, 0] = np.clip(features[:, 0], 0.0, 1.0)
    # features[:, 1] = np.clip(features[:, 1], 0.0, 1.0)
    # features[:, 2] = np.clip(features[:, 2], 0.0, 80.0)

    # features[:, 0] = (features[:, 0] - 7.2558375e-02) / (2.6764875e-02**0.5)
    # features[:, 1] = (features[:, 1] - 0.1) / (0.01**0.5)
    # features[:, 2] = (features[:, 2] - 20.0) / (1500**0.5)

    # features[:, 3] = np.clip(features[:, 3], -80.0, 80.0)
    # features[:, 4] = np.clip(features[:, 4], -80.0, 80.0)
    # features[:, 5] = np.clip(features[:, 5], -5.0, 10.0)

    # features[:, 3] = (features[:, 3] - 1.5672500e00) / (3.0740625e02**0.5)
    # features[:, 4] = (features[:, 4] - 9.8824875e-01) / (2.1913250e02**0.5)
    # features[:, 5] = (features[:, 5] - 1.4) / (1.0**0.5)
    ...
    features[2] = np.clip(features[2], -80.0, 80.0)
    features[3] = np.clip(features[3], -80.0, 80.0)
    features[4] = np.clip(features[4], -5.0, 10.0)

    features[2] = (features[2] - 1.5672500e00) / (3.0740625e02**0.5)
    features[3] = (features[3] - 9.8824875e-01) / (2.1913250e02**0.5)
    features[4] = (features[4] - 1.4) / (1.0**0.5)
    return features
