...
def benchmark(
    ...
    num_confidence_samples = 2
    # min_confidences = np.linspace(0.05, 0.7, num_confidence_samples, endpoint=True)
    min_confidences = (0.1,)
    ...
    for t in min_confidences:
        ...
        # model.post_processing_config["nms_threshold"] = 0.01
        model.post_processing_config["min_confidence"] = 0.05
        ...
        for i, index in enumerate(tqdm(indices)):
            ...
            if log_outputs:
                output_dir = Path.home() / "predictions"
    ...
    # metrics = pl.concat(metrics_list).select(
    #     (pl.col("category"), pl.col("*").exclude("category").round(3))
    # )
    if dataset_name == "av2":
...
# def _erf():
#     activation = output[select_idx].sum()
#     activation.backward()
#     grad = feat.grad.norm(dim=-1).clone()
#     grad = (grad - grad.min()) / (grad.max() - grad.min())
#     grad = grad.detach().cpu().numpy()
#     grad = np.clip(grad * 10, 0, 1)
#     grad = (grad * 255).astype(np.uint8)
#     cmap = cv2.cvtColor(
#         cv2.applyColorMap(grad[:, None], cv2.COLORMAP_JET), cv2.COLOR_BGR2RGB
#     )
...
if __name__ == "__main__":
    benchmark(
        ...
        project_name="your-wandb-project-here",
        entity="user",
        version=0,
        dtype=torch.float16,
        use_oracle=False,
