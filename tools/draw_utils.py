...
def draw_on_bev(
    ...
    for i, (vertices, score) in enumerate(zip(vertices, scores)):
        vertices, _ = world_to_image(vertices, max_side, meter_per_px)

        vertices = vertices.numpy()[:, [1, 0]]
        img = cv2.polylines(
            img=img,
...
def build_bev(xyz: torch.Tensor, max_side: float, meter_per_px: float) -> torch.Tensor:
    ...
    xyz, nm = world_to_image(xyz, max_side, meter_per_px)
    x, y = xyz[:, :2].t().long()

    bev = torch.full(size, dtype=torch.uint8, fill_value=255, device=xyz.device)

    dists = dists[mask]
    dists = dists / dists.max() * 255.0

    colors = TURBO_R.to(dists.device)[:, dists.long()].t()[nm]
    bev[x, y, :3] = colors
    return bev
...
def draw_detections(
    ...
    for batch_index in [batch_index]:
        ...
        for key, multiscale_outputs in network_outputs["head"].items():
            ...
            valid = torch.linalg.norm(cart, dim=0, keepdim=True) > 0

            dists = cart.norm(dim=0, keepdim=True)
            ...
            features = data["features"][batch_index]
            features = [
                normalize_apply_colormap(
                    img=x[None].unsqueeze(0),
                    colormap=VIRIDIS.to(features.device),
                    mask=mask,
                )
                for x in features
            ] + [dists]

            dists = torch.cat(features, dim=1)

            valid = valid.repeat((1, len(features), 1))
            # dists += ((~valid) * 255.0).byte()
            # breakpoint()
            # dists[(~valid).nonzero()] = 255.0
            # breakpoint()
            # dists[valid.repeat_interleave(len(features, dim=1))]

            if stride == 1:
                xyz = torch.as_tensor(cart.reshape(3, -1).transpose(1, 0).clone())
                bev = build_bev(xyz, max_side, meter_per_px).cpu().numpy()
            ...
            if W < SW:
                ...
                bev = F.pad(bev, [left, right, 0, 0], value=255.0)
